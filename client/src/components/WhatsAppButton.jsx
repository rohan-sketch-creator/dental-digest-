...
const WhatsAppButton = ({ appointment, whatsappLink }) => {
    if (!appointment) return null;

    // Generate link if not provided
    const clinicPhone = '1234567890'; // placeholder
    const message = encodeURIComponent(
        `🦷 *SmileCare Dental — Appointment Confirmation*\n\n` +
        `📋 *Booking ID:* ${appointment._id}\n` +
        `👤 *Patient:* ${appointment.patientName}\n` +
        `🏥 *Service:* ${appointment.service}\n` +
        `👨‍⚕️ *Dentist:* Dr. ${appointment.dentistName}\n` +
        `📅 *Date:* ${appointment.date}\n` +
        `🕐 *Time:* ${appointment.timeSlot}\n` +
        `${appointment.notes ? `📝 *Notes:* ${appointment.notes}\n` : ''}\n` +
        `Please confirm this appointment. Thank you! 😊`
    );
    const link = whatsappLink || `https://wa.me/${clinicPhone}?text=${message}`;
